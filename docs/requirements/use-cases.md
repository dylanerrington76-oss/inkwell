...
2. The system autosaves the draft periodically while the author writes (negotiated scope).
...
- **1a.** Author navigates away mid-draft: the most recent autosave is retained (depends on the negotiated autosave scope).
...
- **2a.** No posts exist yet: system shows an explicit empty state, not a blank screen.
